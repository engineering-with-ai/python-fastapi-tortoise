...
## FastAPI + Tortoise ORM Guidelines ⚡
...
### Database & ORM
- **Use Tortoise ORM** with `asyncpg` for async PostgreSQL access
- **Register models via `RegisterTortoise`** in the app module with `generate_schemas=True`
- **Use `model_dump(exclude_unset=True)`** for partial updates from DTOs
- **Use async model methods** — `Model.create()`, `Model.filter()`, `Model.all()`, `Model.delete()`
